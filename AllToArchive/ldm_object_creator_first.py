...
from utils.util_flogging import trace_method
...
class ObjectCreator:
    ...
    def create(self, data_dict: Dict[str, Any]) -> Any:
        ...
        if not cls:
            print(
                f"MappingError: Unknown type: {type_name}. Available types: {list(self.class_map.keys())}"
            )
    ...
    def _prepare_kwargs(self, cls: Any, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field_name, field_value in data_dict.items():
            ...
            if field_name in hints:
                ...
                print(
                    f"MappingError: dict field {field_name} has no matching model field for {cls.__name__}"
                )
    ...
    def _process_value(self, value: Any, expected_type: Any) -> Any:
        ...
        if isinstance(expected_type, str):
            ...
            if hasattr(self.model_module, expected_type):
                ...
                print(
                    f"MappingError: expected type is a string, but not found in module - {expected_type}"
                )
        ...
        # Handle complex types (dictionaries that need to be converted to objects)
            ...
            if (
                ...
                and hasattr(self.model_module, "OneLiner")
            ):
                # Create a OneLiner or Paragraph based on the type
                print(
                    f"MappingError: Need special processing. Checking for creation {value['type_label']} from dictionary: {value}"
                )
                        ...
                        for item in value["content"]:
                            if item.get("type_label") == "PARAGRAPH":
                                ...
                                if "content" in item and isinstance(
                                    ...
                                    para_text = " ".join(
                                        [c.get("content", "") for c in item["content"]]
                                    )
                                    paragraphs.append(
                                        self.model_module.Paragraph(para_text)
                                    )
                                elif "content" in item:
                                    paragraphs.append(
                                        self.model_module.Paragraph(
                                            str(item["content"])
                                        )
                                    )
            ...
            if (
                ...
                and expected_type.__name__ in self.class_map
            ):
                ...
                for field in fields(expected_type):
                    if field.name in value:
                        ...
                        kwargs[field.name] = self._process_value(
                            value[field.name], field_hint
                        )
                ...
                try:
                    ...
                    print(
                        f"MappingError: failed creating {expected_type.__name__}: {str(e)}"
                    )
                    return value
        ...
        # Handle simple types that don't need conversion
            ...
            if inspect.isclass(expected_type) and expected_type.__name__ in (
                "UpperCamel",
                "LowerCamel",
                "CamelCase",
            ):
