...
from lark import (
    ...
    Discard,
    v_args,
)  # Transformer, Discard, v_args might be used in the Transformer
from PyQt5 import QtWidgets, QtCore, QtGui
import utils.utils_gui as utils_gui
...
class TextEdit(utils_gui.CodeEditor):
    ...
    def keyPressEvent(self, e: QtGui.QKeyEvent) -> None:
        ...
        """
        if (
            self.mode in ("grammar", "transformer")
            and settings["options.edit.tabs.replace"]
            and e.key() == QtCore.Qt.Key_Tab
        ):
    ...
    def save(self):
        ...
        if self.file:
            ...
            # show file save dialog
            file = QtWidgets.QFileDialog.getSaveFileName(
                self, "Save File", settings["path.current"], self.file_filter
            )
            file = file[0]
...
class TabWidget(QtWidgets.QTabWidget):
    ...
    def keyPressEvent(self, e: QtGui.QKeyEvent) -> None:
        ...
        if (
            ...
            and QtCore.Qt.Key_1 <= e.key() < QtCore.Qt.Key_1 + min(10, self.count())
        ):
...
class SettingsWindow(QtWidgets.QWidget):
    ...
    def __init__(self, parent):
        ...
        self.lark_start_rule_edit = QtWidgets.QLineEdit(
            settings["options.lark.starting_rule"]
        )
        ...
        self.edit_number_spaces.setValue(
            settings["options.edit.tabs.replacement_spaces"]
        )
        ...
        self.edit_show_line_numbers.setChecked(
            settings["options.edit.show_line_numbers"]
        )
        self.edit_automatic_save = QtWidgets.QComboBox()
        self.edit_automatic_save.addItems(automatic_save_preferences)
        self.edit_automatic_save.setCurrentIndex(
            settings["options.edit.automatic_save_preference"]
        )
    ...
    def closeEvent(self, a0: QtGui.QCloseEvent) -> None:
        ...
        settings["options.edit.tabs.replacement_spaces"] = (
            self.edit_number_spaces.value()
        )
        settings["options.edit.wrap_lines"] = self.edit_wrap_lines.isChecked()
        settings["options.edit.show_line_numbers"] = (
            self.edit_show_line_numbers.isChecked()
        )
        settings["options.edit.automatic_save_preference"] = (
            self.edit_automatic_save.currentIndex()
        )
...
class LarkHighlighter(QtGui.QSyntaxHighlighter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rules = [
            (QtCore.QRegularExpression(pattern), LarkHighlighter.styles[format])
...
class SearchWidget(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.fmt_current = utils_gui.createTextCharFormat(
            QtCore.Qt.white, QtCore.Qt.blue
        )
        ...
        if search_text:
            ...
            if mode == "forward":
                ...
                cursor = document.find(
                    search_text, cursor, QtGui.QTextDocument.FindBackward
                )
    ...
    def update(self):
        ...
        self.target.setExtraSelections(
            self.all_extra_selections + self.current_extra_selection
        )
...
class MainWindow(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in range(number_tabs):
            ...
            self.transformer_highlighters.append(
                utils_gui.PythonHighlighter(edit.document())
            )
            if files[i]:
                edit.load(files[i])
        ...
        self.parsed.setTextInteractionFlags(
        ...
        self.transformed.setTextInteractionFlags(
            QtCore.Qt.TextSelectableByMouse | QtCore.Qt.TextSelectableByKeyboard
        )
        ...
        self.help_window.setMinimumSize(
            int(minimal_window_size[0] * 0.8), int(minimal_window_size[1] * 0.8)
        )
        ...
        self.help_window.setFontPointSize(
            10
        )  # for some reason the default is quite small
        cursor.clearSelection()
        self.help_window.setTextCursor(cursor)
        self.help_window.setTextInteractionFlags(
    ...
    def set_parsed_error(self, text):
        ...
        self.parsed.appendHtml(
            '<font color="red"><strong>{}</strong></font>'.format(text)
        )
                    ...
                    if edit.is_modified():
                        if settings["options.edit.automatic_save_preference"] == 1:
                            ...
                            answer = QtWidgets.QMessageBox.warning(
                                ...
                                "{} tab {} contains unsaved modifications. Save?".format(
                                    name, i + 1
                                ),
...
def update(main_window: MainWindow):
    ...
    try:
        parser = Lark(
            grammar,
            start=settings["options.lark.starting_rule"],
            parser=lark_parsers[settings["options.lark.parser"]],
            debug=False,
        )
    ...
    try:
        ...
        transformed_object = MyTransformer().transform(
            parsed_tree
        )  # MyTransformer should be a resolved reference at runtime
        if isinstance(transformed_object, (list, tuple)):
            main_window.set_transformed("\n".join((str(x) for x in transformed_object)))
        elif isinstance(transformed_object, dict):
            main_window.set_transformed(
                "\n".join(
                    ("{}: {}".format(k, v) for k, v in transformed_object.items())
                )
            )
        else:
...
if __name__ == "__main__":
    ...
    if (
        settings["window.size.width"] > screen_size.width()
        or settings["window.size.height"] > screen_size.height()
    ):
        settings["window.size.width"] = min(
            settings["window.size.width"], screen_size.width()
        )
        settings["window.size.height"] = min(
            settings["window.size.height"], screen_size.height()
        )
