...
import re
from utils_pom.util_antlr import path_for, input_doc_path
...
def do_markup(ppass: str, grammar: str, document: str) -> None:
    ...
    if not input_path.exists():
        raise FileNotFoundError(
            f"Input file not found for {grammar} - {document}: {input_path}"
        )
...
def markup_text_lines(lines: List[str], debug=False) -> List[str]:
    ...
    for line in lines:
        ...
        if line_type == "blank":
            releaseAny(
                "collecting_text"
            )  # a blank line closes any collection, except for text
        ...
        #  will determine whether the value should be wrapped in text markers
            ...
            if (
                normalize_label(label) not in normal_non_text_annotations
            ):  # I.E has a text value
