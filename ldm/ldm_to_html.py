...
def add_html_simple_content(obj_type, obj, html_h):
    ...
    try:
        ...
        html_h.append(
            div(content_h, class_ = f"{obj_type} mdhtml")
        )
    except Exception:
        print(f"failed on simple content: obj type is {obj_type}, content is...\n{content}")
...
def add_html_clause_value(key, value, html_h):
    value_h = span(
                    dict_to_html(value),
                    class_ = f"{key}_value value"
    )    
def add_classed_value_html(key, value, html_h):
    html_h.append(
        span(dict_to_html(value), class_ = f"{key} value")
    )
