...
def object_to_dict(obj, max_depth=20, current_depth=0, visited=None):
    ...
    if isinstance(obj, list):
        return [
            object_to_dict(item, max_depth, current_depth + 1, visited) for item in obj
        ]

    # Handle special LDM types - add specific handling for TypedLine objects
    if hasattr(obj, "type_label") and hasattr(obj, "content"):
        result = {"type": obj.type_label}
        if isinstance(obj.content, list):
            result["content"] = object_to_dict(
                obj.content, max_depth, current_depth + 1, visited
            )
        else:
    ...
    if isinstance(obj, dict):
        return {
            key: object_to_dict(value, max_depth, current_depth + 1, visited)
...
def model_to_json(model, indent=2, max_depth=20):
    ...
    """
    return json.dumps(
        object_to_dict(model, max_depth=max_depth), indent=indent, ensure_ascii=False
    )
