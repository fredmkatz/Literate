...
import re
def parse_trivial(input: str) -> str:
    return input
...
def parse_attribute_reference(input_str: str) -> dict:
...
def test_parsers():
    ...
    assert parse_attribute_reference("Class.Attribute") == {
        "class_name": "Class", 
        "attribute_name": "Attribute"
    }
    
    # Test parse_input_line
    assert parse_input_line("_ **Component** - A building block").get("line_type") == "Class"
    assert parse_input_line("- **name** - The name (Type)").get("line_type") == "Attribute"
    assert parse_input_line("🔄 **Default**: value").get("emoji") == "🔄"
    
    # Test parse_full_header
    header_result = parse_full_header("_ **Component** - An element (Type)")
