...
from utils.util_flogging import trace_decorator, trace_method
...
class FieldType(ABC):
        ...
        if bracket_pos > 0:
            ...
            # Handle Union specially (could be Optional)
                ...
                if has_none and len(non_none_types) == 1:
                    ...
                    # It's a true Union
                    element_types = [
                        cls.create(item) for item in content if item.strip() != "None"
                    ]
                    return UnionType(
                        field_type_str,
            ...
            if operator in cls._type_registry:
                return cls._type_registry[operator].from_content(
                    field_type_str, content, is_optional
                )
    ...
    def display(self, indent: int = 0) -> str:
        ...
        indent_str = " " * indent
        lines = [
            f"{indent_str}Field Type: {self.raw_string} ({self.__class__.__name__})"
        ]
...
class SimpleType(FieldType):
    ...
    def __init__(self, raw_string: str, is_optional: bool = False):
        ...
        self._is_primitive = raw_string in {
            "str",
            "int",
            "float",
            "bool",
    ...
    @classmethod
    def from_content(
        cls, raw_string: str, content: List[str], is_optional: bool = False
    ) -> SimpleType:
...
class ListType(FieldType):
    ...
    def __init__(
        self, raw_string: str, element_type: FieldType, is_optional: bool = False
    ):
        super().__init__(raw_string, is_optional)
...
class DictType(FieldType):
    ...
    def value_phrase(self, metadata: dict) -> str:
        ...
        rule = dict_template.replace("{key_phrase}", key_phrase).replace(
...
class TupleType(FieldType):
    ...
    def __init__(
        self, raw_string: str, element_types: List[FieldType], is_optional: bool = False
    ):
        super().__init__(raw_string, is_optional)
...
class UnionType(FieldType):
    ...
    def from_content(
        ...
        element_types = [
            FieldType.create(item) for item in content if item.strip() != "None"
        ]
