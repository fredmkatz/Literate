...
from utils.util_json import clean_dict, update_nested_dict
from utils.util_fmk import read_yaml_file, write_yaml, write_text
...
class PomDict:
    ...
    def __str__(self):
        ...
        yamlstr = yaml.dump(
            clean_dict(self.the_dict), default_flow_style=False, sort_keys=False
        )
    ...
    def fill_in_defaults(
        self, target_meta: Dict, defaults: Dict, tag_filter: Dict
    ) -> Dict:
    ...
    def fill_in_field_defaults(
        self, target_meta: Dict, defaults: Dict, field_suffix: str
    ) -> Dict:
        for def_key, def_value in defaults.items():
    ...
    def get_class_metadata_with_defaults(self, class_name: str) -> Dict[str, Any]:
        ...
        resolved_class_meta = self.fill_in_defaults(
            class_metadata, defaults, Class_Metas
        )
    ...
    def get_field_metadata_with_defaults(
        self, class_name: str, field_name: str, suffix: str
    ) -> Dict[str, Any]:
        """
        ...
        resolved_field_meta = self.fill_in_field_defaults(
            field_metadata, defaults, suffix
        )
...
def get_meta_attributes_with_values(cls):
    ...
    if hasattr(cls, "Meta"):
        ...
        goodvars = {
            key: value for key, value in meta_vars.items() if not key.startswith("__")
        }
...
class PomMeta:
    ...
    def resolve_metas(self, live_meta: PomDict, external_meta: PomDict) -> PomDict:
        ...
        for class_name in class_names:
            ...
            resolved_class_meta = update_nested_dict(
                live_class_meta, external_class_meta
            )
        ...
        legend = []
        legend.append(
            f"## Resolved metadata from {live_meta.name} and {external_meta.name}"
        )
    ...
    def gather_live_metadata(self, model_module, model_name, format_name, classes):
        ...
        self.live = PomDict(
            "Live", live_meta, legend=[f"# extracted from the {model_name} model"]
        )
