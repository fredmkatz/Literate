...
from utils.util_flogging import flogger, trace_decorator, trace_method
from utils.util_json import as_json, clean_dict
from utils.class_casing import UpperCamel, LowerCamel, NTCase, TokenCase
from pom.class_field_type import FieldType
from pom.class_field_type import (
    field_terminals,
...
from utils.util_fmk import write_yaml

from pom_config import primitive_terminals, pmark_named
from pom.class_abstract_grammar import (
    AbstractGrammar,
    AbstractSection,
    AbstractClause,
    AbstractDisjunction,
...
try:
    from pom.class_grammar import Grammar
...
class PomGrammarGenerator(grammar_base):
    ...
    def __init__(self, pom_config: PomConfig, name="PresentableGrammar"):
        ...
        self.class_clause_name = (
            {}
        )  # maps class name to the name of the class clause rule
    ...
    def generate_grammar(self, model_module, model_name, format_name) -> str:
        ...
        self.pom_meta.gather_live_metadata(
            model_module, model_name, format_name, classes
        )
    ...
    def _derive_class_hierarchy(self, classes: Dict) -> Dict:
        ...
        print(path_dict)
        classes_by_topol = sorted(
            self.class_hierarchy.keys(), key=lambda x: self.class_hierarchy[x]["path"]
        )
    ...
    def _generate_grammar_terminals(self, abstract_terminals: AbstractSection):
        ...
        for qf_name in sorted(field_name_literals):
            ...
            abstract_terminals.add_t_rule(
                f"{qf_name}", f"/{pattern}/{case_insensitive}", priority=9
            )
        ...
        for terminal in sorted(field_terminals):
            if terminal not in pmark_named and terminal not in {
                ...
                abstract_terminals.add_t_rule(
                    terminal, f'"{terminal_pattern}"{case_insensitive}', priority=8
                )
        ...
        for class_name, info in self.class_hierarchy.items():
            ...
            if len(bases) == 1:
                ...
                if base_pattern == token_pattern:
                    abstract_terminals.add_t_rule(
                        class_name, terminal_class_name(base_name)
                    )
                    continue
    ...
    def _gen_rules_for_class(self, abstract_classes: AbstractSection, class_name, cls):
        ...
        abstract_classes.add_comment(
            f"          ========== {UpperCamel(class_name)} =========="
        )
        ...
        (newfields, redefined_fields, inherited, inherits_from) = self._assess_fields(
            cls
        )
        ...
        if full_template:
            ...
            full_template_clause = self._gen_full_class_by_template(
                class_name, pom_full_template
            )
            ...
            if header_template:
                ...
                header_rule = abstract_classes.add_nt_rule2(
                    header_clause_name, header_clause
                )
                flogger.infof(f"Header clause is: {header_clause}")
            ...
            ancestor_clauses = list(
                AbstractClause(self.class_clause_name[base]) for base in inherits_from
            )
            if ancestor_clauses:
                flogger.infof(
                    f"Ancestor clauses for {class_name} are {ancestor_clauses}"
                )
        ...
        if header_clause and class_clauses_name:
            disjuncts.append(
                AbstractClause(
                    str(header_clause) + "  " + "(" + class_clauses_name + ")*"
                )
            )
        elif header_clause:
        ...
        abstract_classes.add_nt_rule2(
            class_name, AbstractDisjunction(disjuncts), priority=depth + 1
        )

        if fields_needing_rules:
            abstract_classes.add_comment(
                f"  ... value rules for {NTCase(class_name)}  ..."
            )
            ## add the value rules
    ...
    # @trace_method
    def _gen_full_class_by_template(
        self, class_name, template: PomTemplate
    ) -> AbstractClause:
        """
        Generate a header rule for a class based on a template.
    ...
    def _gen_field_clauses(
        self, class_name, cls, inherited: List[str]
    ) -> List[AbstractClause]:
        """
    ...
    def _generate_field_clause(
        ...
        field_meta = self.pom_meta.resolved.get_field_metadata_with_defaults(
            class_name, field_name, fieldType.suffix()
        )
        # flogger.infof(f"Field meta  for {class_name}.{field_name} is: {field_meta}")
        field_clause_template = field_meta.get(
    ...
    def _generate_field_value_rule(
        ...
        metadata = self.pom_meta.resolved.get_field_metadata_with_defaults(
            class_name, field_name, fieldType.suffix()
        )
        # Rule name
    ...
    def get_templates(self):
        ...
        for class_name in self.class_hierarchy.keys():
            class_meta = self.pom_meta.resolved.get_class_metadata_with_defaults(
                class_name
            )
