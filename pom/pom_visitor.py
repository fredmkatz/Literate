...
from utils.more_casing import to_snake_case, to_upper_camel, to_lower_camel
...
class PomVisitor(Interpreter):
    ...
    def _is_class_node(self, node_name):
        ...
        return (
            not self._is_value_node(node_name)
            and not node_name.endswith("_clause")
            and not node_name.endswith("_header")
            and not node_name.endswith("_body")
        )
    ...
    def _handle_class_node(self, tree, node_name):
        ...
        if not class_type:
            ...
            self.diagnostics.add_warning(
                f"Unknown class type: {node_name}", self._get_location(tree)
            )
            return self.visit_children(tree)
    ...
    def _parse_value_node_name(self, node_name):
        ...
        class_name = base_name[
            : -len(field_name) - 1
        ]  # Remove field name and underscore
